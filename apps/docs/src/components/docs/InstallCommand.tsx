...
/**
 * `compact` keeps the manager tabs but drops the "Local package" caption, since
 * the hero says that in its own words. Every manager installs the same package
 * from the same registry, so the choice is only about which line you paste.
 */
export default function InstallCommand({ compact = false }: { compact?: boolean } = {}) {
  ...
  return <div className={`install-command${compact ? ' is-compact' : ''}`}>
    <div className="install-toolbar"><div className="segmented" aria-label="Package manager">{managers.map(m => <button type="button" key={m} aria-pressed={manager === m} onClick={() => {setManager(m);setStatus('');}}>{m}</button>)}</div>{!compact && <span>Local package</span>}</div>
